...
- Раскладка, которая не меняется сама. Координаты узлов живут на
  ...
  бросит движок.
- Поставить узел — не то же самое, что на него посмотреть.
  Перетаскивание двигает вид; в режиме **«Расстановка»** оно ставит
  узел и оставляет его там, вне досягаемости движка раскладки, а
  правая кнопка отпускает обратно. То же делает клавиша `P`.
  Закреплённый узел по-прежнему можно таскать — закрепление говорит
  «физика сюда не лезет», а не «руками не трогать», — а в шапке
  написано, какая часть карты поставлена человеком, а не разложена
  движком.
- Двойной клик по коммутатору (или по любому узлу-группе) выделяет
  ...
  двигает всё выделение разом. Меню по правой кнопке работает со всем
  выделением: поставить, отпустить, открыть карточку, открыть порты.
...
| v0.7.2 | Контекстное меню узла с диагностикой и своими командами оператора |
| v0.7.3 | Аутентификация |
