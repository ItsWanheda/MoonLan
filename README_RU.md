...
- Раскладка, которая не меняется сама. Координаты узлов живут на
  ...
  смотрящих на неё, должны видеть одну схему. Перетащенный узел
  закрепляется там, где его отпустили, и сохраняется сам; он помечен,
  чтобы карта отвечала на вопрос «это MoonLan так разложил или я так
  поставил», и открепляется из карточки или правой кнопкой. «Сохранить
  раскладку» запоминает положение всех узлов и делает картинку
  воспроизводимой. Узел, которого в сохранённой раскладке нет,
  нарисован пунктиром и посчитан в шапке — расхождение видно сразу, а
  не при печати. Позиция пропавшего узла сохраняется: устройство,
  выключенное на ночь, не разобрали, — и забывается она, только если И
  старше `layout_keep_days`, И узла на карте больше нет. Всё это
  показывает `python -m moonlan.diag --layout`.
...
| v0.7 ✓ | Раскладка, которая не меняется сама: координаты на сервере, ручное размещение, видимое расхождение |
| v0.7.1 | Экспорт в PDF и Draw.io, импорт сведений по MAC-адресам |
